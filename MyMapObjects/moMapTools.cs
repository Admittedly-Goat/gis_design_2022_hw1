...
namespace MyMapObjects
{
    ...
    public static class moMapTools
    {
        ...
        {
            return GetDistance(point.X, point.Y, pointOverlapped.X, pointOverlapped.Y) <= tolerance;
        }
        ...
        public static bool IsPointWithinMultiPolygon(moPoint point, moMultiPolygon multiPolygon)
        {
            ...
            for (int i = 0; i <= sPartCount - 1; i++)
            {
                sIntersectionCount += GetIntersectionCountBetweenRayAndPolygon(point, multiPolygon.Parts.GetItem(i));
            }
        ...
        {
            int sPointCount = points.Count;
            List<double> sDises = new List<double>
            {
                0  //第一个点至起点的距离
            };     //所有顶点至起点的距离
